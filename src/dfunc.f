	subroutine dfunc(p,psfs,npsize,npnmax,grad,fp)
...
! Calculate the matrix elements.
     ...
     *			    pint(k,m)*pint(k,n)/sigdvalue(k,ib(i))**2
	    ...
	    AA = matmul(transpose(A), A)     ! apparently matmul takes
	    call matinv(AA,Ai,mblend,ok)    ! R*4 and R*8 -- JWF B30501
...
! Calculate residuals.
     ...
     *		sigdvalue(1:nv,ib(i)))**2)
	    ...
	    do n = 1,nblend
	      if (goodcomp(n)) then
		nc = nc + 1
		gsum(i,nc) = -2.0*fluxb(nc)*sum(pintx(1:nv,n)*
     *		    (dvalue(1:nv,ib(i)) - model(1:nv))/
     *		    sigdvalue(1:nv,ib(i))**2)
		gsum(i,mblend+nc) = -2.0*fluxb(nc)*sum(pinty(1:nv,n)*
     *		    (dvalue(1:nv,ib(i)) - model(1:nv))/
     *		    sigdvalue(1:nv,ib(i))**2)
	      endif
	    enddo
