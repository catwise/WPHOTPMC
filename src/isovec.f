 	SUBROUTINE isovec (nx,ny,array,R,x0,y0,rmax,
     1   ratio,phimax,nmax,iso,nx_iso,ny_iso,nth,csig)

	implicit integer (i-n)
        implicit real*4 (a-h)
        implicit real*4 (o-z)

        real*4 rad(2000),rval(2000),ddx(2000),ddy(2000)
c	real*4 larray (261*261)
	integer*2 iso(nmax,nmax),ibf(261,261)
c	character*72 fname,tick
        real*4 array(nx,ny)
        logical debug
	...
	zlim = nth * csig
	if (debug) write (6,*) 'ISOVEC ',zlim,R,nmax
...
c	write (6,*) 'QUAD 1 begin ',theta,i0,j0
	 ...
	 if (theta.lt.45.0) then
		...
		if (val.lt.zlim-0.5) goto 50

 50	 continue


c	write (6,*) 'here quad 1 ',n,theta,zlim
c	write (6,*) rad(10),rval(10)
c	write (6,*) rad(50),rval(50)
c	write (6,*) rad(90),rval(90)
c	write (6,*) rad(n),rval(n)
...
c now find n-sigma perimeter
 ...
 51	if ((idex1.eq.0).and.(idex2.eq.0)) then
		if ((rval(n).gt.zlim).and.(itry.lt.10)) then
c  			too bright, change the isophote limit
			zlim = zlim + csig
			goto 777
		endif
	endif
...
c	write (6,*) 'QUAD 2 begin ',theta,i0,j0
	 ...
	 if (theta.gt.135.0) then
		...
		if (val.lt.zlim-0.5) goto 60

 60	 continue

c	write (6,*) 'here quad 2 ',n
...
c now find n-sigma perimeter
 ...
 61     if ((idex1.eq.0).and.(idex2.eq.0)) then
                if ((rval(n).gt.zlim).and.(itry.lt.10)) then
c                       too bright, change the isophote limit
                        zlim = zlim + csig
                        goto 777
...
c	write (6,*) 'QUAD 3 begin ',theta,i0,j0
	 ...
	 if (theta.lt.225.0) then
		...
		if (val.lt.zlim-0.5) goto 70

 70	 continue


c	write (6,*) 'here quad 3 ',n
...
c now find n-sigma perimeter
 ...
 71     if ((idex1.eq.0).and.(idex2.eq.0)) then
                if ((rval(n).gt.zlim).and.(itry.lt.10)) then
c                       too bright, change the isophote limit
                        zlim = zlim + csig
                        goto 777
...
c	write (6,*) 'QUAD 4 begin ',theta,i0,j0
	 ...
	 if (theta.gt.315.0) then
		...
		if (val.lt.zlim-0.5) goto 80

 80	 continue


c	write (6,*) 'here quad 4 ',n
...
c		write (6,*) L,ddx(l),ddy(l),rad(L),val
 ...
 81     if ((idex1.eq.0).and.(idex2.eq.0)) then
                if ((rval(n).gt.zlim).and.(itry.lt.10)) then
c                       too bright, change the isophote limit
                        zlim = zlim + csig
                        goto 777
...
c        do iii=1,nmax
c                  ic = ic + 1
c                  larray(ic) =  iso(iii,jjj)
c        enddo
c        enddo
c        fname='isomask0'
c        call writeimage (nmax,nmax,larray,ic,tick,fname,iii,jjj,crv1,crv2)


c	write (6,*) 'down to come ellipse parameters',iso(60,60),iso(200,60),iso(60,200),iso(200,200)
...
c com ellipse parameters
	...
	dmaxy = 0
        ...
        if (rmax.ge.1) then
c due to pixelation, rmin uncertainty of 1
                ratio = (rmin+1.) / rmax
