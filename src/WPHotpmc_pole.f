...
c
      call signon('WPHotpmc_pole')          ! JWF B30507
      if (IArgc() .eq. 0) then
        print *,'For instructions on running this program: ask Tom Jarrett'
        call signoff('WPHotpmc_pole')
        stop
      end if
c
...
c load the FITS header into memory
     ...
     1               'ib,j=',ib,j,'frame=',fram(j,ib)(1:L)
                ...
                if (zexist) then
                 ...
                 wflag(j,ib) = 0
                 HDR0 (j,ib) = 'null'
c                call exit(64)         ! JWF B80524, B80625
                 goto 6000
c                     write (6,*) '  exiting '
c                     call exit(9)
                  endif
...
c                  if (ib.eq.1) write (86,*) m,XSCprox(m)
 ...
 910    close (11)
     ...
     +   print *,'No. of mmm fallbacks:                ', NInt(fDup*nFallbackMMM)         ! JWF B30606
        ...
        call signoff('WPHotpmc_pole')        ! JWF B30507
