	real*4 function func_pm(p,psfs,npsize,npnmax,nDoF_pm,dbg)
...
! Unpack the parameter vector.
	mblend = ntrue(goodcomp,nblend)
        if (dbg) then
          print *,'============================='
          print *,'func_pm(51): nblend, mblend:', nblend, mblend ! JWF dbg
        end if
	xi = 0.
	eta = 0.
	nc = 0
	...
	pmd = p(2*mblend+2)/yr	! deg/day
        if (dbg) then
          print *,'func_pm(65): p(2*mblend+1), p(2*mblend+2):',p(2*mblend+1), p(2*mblend+2)! dbg
          print *,'func_pm(66): pmr, pmd:',pmr,pmd ! dbg
        end if
	...
	do iframe = 1,nframes
		...
		if (n==icp) then
		    ...
		    dec8 = dec8 + pmd*tframe(iframe)
                    if (dbg) then
                      print *,'func_pm(91): n, nblend, icp:',n, nblend, icp ! JWF dbg
                      print *,'func_pm(92): iframe, tframe(iframe):',iframe, tframe(iframe) ! JWF dbg
                      print *,'             ra8,dec8:',ra8, dec8
                      print *,'func_pm(93): dRA, dDec:',pmr*tframe(iframe)/cos(dtor*deccan(n)),
     +                                                  pmd*tframe(iframe) ! JWF dbg
                    end if
		endif
        	offscl = -1
		    ...
		    ysource(n,iframe) = y8
                ...
                if (dbg) then
                  print *,'func_pm(110): n, xsource, ysource',n,xsource(n,iframe),ysource(n,iframe) ! JWF dbg
                end if
	    enddo
	enddo
